...
//Application ID and Pasasword will be provided by the Ideamart team after creation of the Ideamart application. After the approval you can use this.
...
//To send SMS we have to talk to this URL
    ...
    //to handle exceptions we surround our code with Try Catch block
        try {
            ...
            $receiver = new SMSReceiver(file_get_contents('php://input'));                //this line will get the data which comes to our php file and using SMSReceiver which in is SMSReceiver class we create new object $receiver
            ...
            list($key, $context) = explode(" ", $content);                                 // As we require the second word after the Keyword here I have used explode function to get the second word which comes after a space

            $random = mt_rand(1, 87);                                                               // We require a random value to be passed along with the url as to get random names

            $json = file_get_contents("https://swapi.co/api/people/$random");               // through this line we access the API url and get the relevant data
            ...
            if ($context == "male") {
                ...
                while ($x < 87) {                                                                       // There are 87 named characters in Star wars
                    if ($gender == "male") {
                        ...
                        $json_name = file_get_contents($Name_url);                                      // To get the data for the specific gender and random character name
                ...
                while ($x < 87) {                                                                        // There are 87 named characters in Star wars
                    if ($gender == "female") {
                        ...
                        $json_name = file_get_contents("https://swapi.co/api/people/$random/");     // To get the data for the specific gender and random character name
            ...
            if (!$name) {                                                                                                   // This If blocks checks if the name is returned null or empty if that is the case the specified message is sent
                $sender->sendMessage('oops something went wrong please try again. May the force be with you', $address);
            }
        } catch (SMSServiceException $e) {
            error_log("Passed Exception-not working " . $e);                                                        // If there is an exception this will not down the exception in error log which may be useful for reference.
        }
